#define FCLOCK 		   100000000
#define PULSE_LENGTH	   1000 //micro-seconds
#define IPP		   10000 //micro-seconds
#define SAMPLE_FREQ	   100000 //samples-per-second
#define FIFOLVL		   8000 
#define FIFOWIDTH	   1 
#define STATE_TIME         5   //in microseconds
#define RX_OFFSET          0   //in microseconds
#define TX_OFFSET	   0   //in microseconds
#define DDS_OFFSET         0   //in microseconds
#define CH0_SAMPLES_TO_COLLECT 	10000
#define CH1_SAMPLES_TO_COLLECT  10000


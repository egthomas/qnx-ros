...
// This is the intterupt handler for the GPS-PCI card. In this software only
// the time compare interrupt and the event capture interrupts are enabled.
// This handler monitors for these two interrupts.
  ...
  //if a time compare or event capture intterupt has occured, then signal
  //the thread monitoring the interrupts (int_thread)
  if (TimeCompareInterrupt || RateSynthInterrupt || EventInterrupt)
    return (&interruptevent);

  //if the time compare or event capture intterupts did not occur, then
  //some other device triggered the interrupts 
  else return (NULL);
}
...
void *int_thread(void *arg)
{
  ...
  // This is the interrupt monitoring thread for the GPC-PCI driver. This
  // thread continially wait for a time compare or event capture interrupt,
  // then implements the necessary code to respond to those interrupts
  ...
  while (1) {
    ...
    if (EventInterrupt) {
      ...
      EventInterrupt = 0;
      //get the time of the event which triggered the interrupt
      temp = get_event_time(&gpssecond,&gpsnsecond,BASE1);
      event.tv_sec  = gpssecond;
      event.tv_nsec = gpsnsecond;
      //set a flag so screen will be updated to display the time of the event
      eventupdate = 1;
    }

...
{
  // This function simply increments the time in 'nexttime' 'increment' ns.
...
float get_lat()
{
  ...
  if (BASE1 != NULL) {
    ...
    degf = (float)deg + ((float)min/60) +
            (((float)sec + 0.1*(float)tenthsec)/3600);

    temp = *((uint32_t*)(BASE1+0x108));
    temp = ((temp&0xff000000) >> 24);

    if (temp == 0x4e)      return ( degf);
    else if (temp == 0x53) return (-degf);
    else {
      fprintf(stderr, "North/South error: %d.\n", temp);
      return (0.);
    }
  } else {
    fprintf(stderr, "Cannot obtain latitude.\n");
    return (0.);
  }
}
...
float get_lon()
{
  ...
  if (BASE1 != NULL) {
    ...
    else if (temp == 0x45) return ( degf);
    else {
      fprintf(stderr, "East/West error: %d.\n", temp);
      return (0.);
    }
  } else {
    fprintf(stderr, "Cannot obtain longitude.\n");
    return (0.);
  }
}
...
float get_alt()
{
  ...
  if (BASE1 != NULL) {
    ...
    else if (temp == 0x2d) return (-altf);
    else {
      fprintf(stderr, "Altitude sign error: %d.\n", temp);
      return (0.);
    }
  } else {
    fprintf(stderr, "Cannot obtain altitude.\n");
    return (-1);
  }
}
...
void *monitor_timecompare(void *arg)
{
  ...
  while (1) { 
    if (displaystat.intervalmode || displaystat.scheduledintervalmode ||
        displaystat.oneshot) {
      ...
      displaystat.settimecomparetime = (float)((stop.tv_sec-start.tv_sec) +
...
int time_greater(struct timespec *timeA, struct timespec *timeB)
{
  if ((timeA->tv_sec > timeB->tv_sec) ||
      ((timeA->tv_sec == timeB->tv_sec) && (timeA->tv_nsec > timeB->tv_nsec))) {
    return (1);
  } else {
    return (0);
  }
...
char* time_string(struct timespec *timeA)
{
  ...
  if (fseconds < 10)
    sprintf(time_s,"%02d/%02d/%04d %02d:%02d:0%08.6f UT", localtime->tm_mon+1,
            localtime->tm_mday, localtime->tm_year+1900, localtime->tm_hour,
            localtime->tm_min, fseconds);
  else
...
void *get_state()
{
  ...
  int cnt, rst, ch;
  int tmp,i,system_tv_sec,gps_tv_sec;
  ...
  tmp = get_sat_stat(displaystat.sv,displaystat.signal);
//      for (i=0;i<6;i+=2){
//      }

  //get GPS position and calculate mean position
  displaystat.lat  = get_lat();
  displaystat.mlat = ((displaystat.mlat*displaystat.poscnt) +
                        displaystat.lat) / ((float)displaystat.poscnt+1);
  displaystat.lon  = get_lon();
  displaystat.mlon = ((displaystat.mlon*displaystat.poscnt) +
                        displaystat.lon) / ((float)displaystat.poscnt+1);
  displaystat.alt  = get_alt();
  displaystat.malt = ((displaystat.malt*displaystat.poscnt) +
                        displaystat.alt) / ((float)displaystat.poscnt+1);

  //get GPS time and system time
  tmp = clock_gettime(CLOCK_REALTIME,&now);
  system_tv_sec = now.tv_sec;
  gps_tv_sec    = displaystat.gpssecond;
  tmp = get_software_time(&displaystat.gpssecond,&displaystat.gpsnsecond,BASE1);
  displaystat.syssecond  = now.tv_sec;
  displaystat.sysnsecond = now.tv_nsec;

  //caclulate time difference
  timediff = (float)(displaystat.syssecond-displaystat.gpssecond) +
             ((float)(displaystat.sysnsecond-displaystat.gpsnsecond))/1e9;
  displaystat.drift = 1e6*(timediff-oldtimediff);
  oldtimediff = timediff;
  displaystat.mdrift = ((displaystat.mdrift*displaystat.poscnt) +
                        displaystat.drift) / ((float)displaystat.poscnt+1);
  displaystat.poscnt++;

  if ((fabs(timediff) > MAX_ERR) && (displaystat.reference_lock) &&
                      (displaystat.gps_lock) && (displaystat.phase_lock)) {
...
//                  close(fd);
      ...
      fprintf(stderr,"ERROR: Time Difference between GPS and System Time "
                     "too large\n");
      fprintf(stderr,"Current Time:\n System: %d %s", system_tv_sec,
              ctime(&system_tv_sec));
//      fprintf(stderr," GPS   : %d %s", gps_tv_sec, ctime(&system_tv_sec));
      fprintf(stderr," GPS   : %d %s", gps_tv_sec, ctime(&gps_tv_sec));
    ...
    tmp = clock_gettime(CLOCK_REALTIME,&now);
    tmp = get_software_time(&displaystat.gpssecond,&displaystat.gpsnsecond,
                            BASE1);
    displaystat.syssecond  = now.tv_sec;
    displaystat.sysnsecond = now.tv_nsec;

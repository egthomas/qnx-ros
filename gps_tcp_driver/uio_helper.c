...
int line_from_file(char *filename, char *linebuf)
{
  ...
  /* SGS: should we be closing the file we opened?! */
...
int uio_get_device_attributes(struct uio_info_t* info)
{
  ...
  while (n--) {
    ...
    if (!dev_attr_filter(fullname)) continue;
...
static struct uio_info_t* info_from_name(char* name, int filter_num)
{
  ...
  num = uio_num_from_filename(name);
...
{
  struct dirent **namelist;
  struct uio_info_t *infolist = NULL;
  struct uio_info_t *infp, *last;
  int n;
...
void uio_single_mmap_test(struct uio_info_t* info, int map_num)
{
  ...
  if (info->maps[map_num].size <= 0) return;
  ...
  map_addr = mmap(NULL, info->maps[map_num].size, PROT_READ, MAP_SHARED,

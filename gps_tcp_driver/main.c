...
/* SGS: 20230314 making code readable...
 * SGS: 20230423 updating and adding year set
 */
...
int main(void) {
  ...
  int rval,buf; 
  int first_gps_lock;
  int temp, pci_handle, i, frame_size, status;
  unsigned int *mmap_io_ptr;
  unsigned int CLOCK_RES;
  struct timespec now,new,old, start_p,stop_p;
...
//  clock_gettime(CLOCK_REALTIME, &start);
  ...
  //INITIALIZE CARD
  if (configured) {
  /* set card for synchronized generator mode,
     GPS reference enabled Daylight Savings Time Disabled */
    *((uint32_t*)(BASE1+0x118)) = 0x90000021; // x90 to address x11B produced
                                              // 10 MHz reference signal output


    /*
     * I THINK YOU ONLY NEED TO DO THIS WHEN THE GPS EPOCH CHANGES: 2000,
     *    2020, 2040, etc.
     */

    // let's check out what is in the Preset Time Registers
    //temp = *((uint32_t*)(BASE1+0x160));
    //printf("*******\n");
    //printf("0x%X\n", temp);
    //printf("%d\n", temp);
    //printf("*******\n");

    //*((uint32_t*)(BASE1+0x160)) = 0x2023;   // set year
    //*((uint32_t*)(BASE1+0x118)) |= 0x04;    // bit 2: Time Preset Ready
    //*((uint32_t*)(BASE1+0x12c)) = 0x04;     // save current setup to EEPROM



    //clear time compare register
    now.tv_sec += 0;  /* this seems stupid... */
    now.tv_nsec = 0;

...
int io_read(resmgr_context_t *ctp, io_read_t *msg, RESMGR_OCB_T *ocb)
{
  ...
  if ((stat = iofunc_read_verify(ctp, msg, ocb, NULL)) != EOK)
    return(stat);
...
int io_write(resmgr_context_t *ctp, io_write_t *msg, RESMGR_OCB_T *ocb)
{
  ...
  if ((stat = iofunc_write_verify(ctp, msg, ocb, NULL)) != EOK)
    return (stat);
   ...
   */
  if ((stat = resmgr_msgread(ctp, buf, msg->i.nbytes, sizeof(msg->i))) == -1)
    return(ENOSYS);
  ...
  switch (msg->i.dcmd) {
    ...
    case ICS660_WRITE_SEQUENCER:
      pdebug("IO_DEVCTL - WRITE_SEQUENCER offset: %x  value: %x\n",
             (uint32_t)rx_data->seq_data.offset,
             (uint32_t)rx_data->seq_data.value);
    ...
    case ICS660_LOAD_FILTER:
      ...
      pdebug("_ICS660_DRV: LOAD_FILTER - \n  CHIP %d\n  CHANNEL %d\n  "
             "F_CORNER %d\n  STATE_TIME %f\n", filter_str.chip,
             filter_str.channel, filter_str.f_corner, filter_str.state_time);
      select_chip(ics660->mod_control_reg,filter_str.chip);
      load_filter_taps(ics660->dc60m_regs, filter_str.channel,
                       filter_str.f_corner, filter_str.state_time);
    ...
    case ICS660_SET_PAGE_REG:
      ...
      *(uint32_t *)ics660->dc60m_regs.input_registers[(int)reg] =
                  (uint32_t) value;
      break;
